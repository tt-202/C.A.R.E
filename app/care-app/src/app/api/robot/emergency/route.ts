...
/** Jetson hardware e-stop → caregiver push (shared secret, not user JWT). */
export async function POST(request: NextRequest) {
  try {
    ...
    let body: RobotEmergencyBody = {};

    try {
      body = (await request.json()) as RobotEmergencyBody;
    } catch {
      body = {};
    }

    const robotId = body.robotId?.trim() || "care-01";
    const reason = body.reason?.trim() || "EMERGENCY_BUTTON";
    const phase = body.phase?.trim() || "unknown";
    ...
    const alert = await publishAndPushCaregiverMealAlert({
      ...
      caregiverName,
      bitesTotal: 0,
      plannedMealTime: `${robotId} / ${phase} / ${reason}`,
      emergency: true,
    });
    ...
    console.error("[robot emergency]", e);
